...
class HadCM3DS(proc.ModelDS):
	...
	def get_lon(self, data_array, mode_lon, value_lon):
		try:
			if mode_lon is None:
				return data_array
			elif mode_lon == "index":
				...
				print(f"New longitude value : {data_array.longitude.values[value_lon]}")
				return data_array.isel(longitude=value_lon)
			elif mode_lon == "value":
				if value_lon is None:
					raise ValueError("To use the value mode, please indicate a value_lon.")
				print(f"New longitude value : {util.lon_to_index(data_array.longitude.values,value_lon)}")
			...
			print(error)
			print("The data_array was not changed.")
	
	def get_lat(self, data_array, mode_lat, value_lat):
		...
		if mode_lat is None:
			...
			print("Mode wasn't recognized. The data_array was not compacted.")
	
	def get_z(self, data_array, mode_z, value_z):
	...
	def get_t(self, data_array, mode_t, value_t):
		...
		if mode_t is None:
			...
			print("Mode wasn't recognised. The data_array was not compacted.")
...
class OCNMDS(HadCM3DS):
	...
	def sst(self, zone=zones.NoZone(), mode_lon=None, value_lon=None, mode_lat=None, value_lat=None, mode_t=None, value_t=None):
